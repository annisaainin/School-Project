...
<head>
  ...
  <title>Ikita Store - Tambah Artikel</title>
...
<body id="page-top">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          <ol class="breadcrumb">
            ...
            <li><a href="data-artikel.php"><b>Data Artikel</b></a> / Tambah Artikel</li>
          </ol> 
          <!-- Page Heading -->                 
          <!-- DataTales Example -->
          <div class="card shadow mb-4">
            ...
            <div class="collapse show" id="collapseCardExample">
              <div class="card-body">
                <form action="pro-tambah-artikel.php" class="form-horizontal" method="POST" enctype="multipart/form-data">
                  
                  <?php 
                    $kode = 'AR';
                    $query2 = mysqli_query($koneksi,"SELECT MAX(id_artikel) as idnew FROM tb_artikel");
                    $row2 = mysqli_fetch_array($query2);

                    $max_id = $row2['idnew'];
                    $max_ids = (int) substr($max_id,3,3);
                    $id_artikel = $max_ids+1;
                    $auto = $kode.sprintf("%04s", $id_artikel);
                  ?>
                  ...
                  <input class="form-control" type="hidden" name="id_artikel" placeholder="ID" value="<?php echo $auto; ?>" readonly/>
                  ...
                  <div class="form-group">
                    <label class="col-lg-9">Judul Artikel</label>
                      <div class="col-lg-12">
                        <input class="form-control form-control-user" placeholder="Masukkan Judul Arikel" type="text" name="judul_artikel" required="">
                      </div>
                  </div>
                  <div class="form-group">
                    <label class="col-lg-9">Tanggal</label>
                      <div class="col-lg-6">
                        <input class="form-control form-control-user" placeholder="Masukkan Tanggal" type="date" name="tgl_artikel" required="">
                      </div>
                  </div>
                  <div class="form-group">
                    <label class="col-lg-9">Isi Artikel</label>
                      <div class="col-lg-12">
                        <textarea class="form-control" id="des" name="isi_artikel" required=""></textarea>
                      </div>
                  </div>
       ...
       $('#multiple_files').change(function(){
        ...
        if(error_images == '')
        {
         $.ajax({
          ...
          {
           $('#preview').load('form-tambah-artikel.php?id=<?php echo $max_id; ?>'+' #preview');
          }
         });
        }
        else
  ...
  function deleteImage(id_gambar) {
        $.ajax({
            ...
            success: function () {
              $('#preview').load('form-tambah-artikel.php?id_gambar=<?php echo $max_id; ?>'+' #preview');
            }
        });
      }
